...
While there is no model which is the best by all criteira the Quadradic model has the lowest AIC and second lowest (behind the linear model) BIC, while also being significantly different in the chi-sqaured test. Looking at the fixed effects the parameters $\gamma_{00}$ (the intercept term) and $\gamma_{10}$ (the linear chnange term) did not drastically change as shown in the table above. In addition the parameter $\gamma_{20}$ (the quadradic term) did not significantly differ
from zero which indicates that the trajectory of depression scores over periods across all individuals is close to linear.
